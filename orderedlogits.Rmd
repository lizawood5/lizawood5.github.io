...
colnames(df) <- c("stages", "income", "acres", "education", "blame.ag", "econ.v", "envt.v", "self.cert", "wqc.perc", "info")
dfr <- round(df[ ,c(3,5,6,7,9)], 2)
dff <- df[ ,c(1,2,4,8,10)]
df <- cbind(dfr, dff)
df <- df %>% dplyr::select(stages, income, acres, education, blame.ag, econ.v, envt.v, self.cert, wqc.perc, info)
```
...
```{r models}
polr.model <- polr(stages ~ acres + income + education +  blame.ag + econ.v + envt.v + info + self.cert + wqc.perc, data = df, Hess = T)
```
...
```{r}
vgam.model.cumulative = vglm(ordered(stages) ~ acres + income + education +  blame.ag + econ.v + envt.v + info + self.cert + wqc.perc, data = df, family=cumulative(parallel = T, reverse = T))

vgam.model.cratio = vglm(ordered(stages) ~ acres + income + education +  blame.ag + econ.v + envt.v + info + self.cert + wqc.perc, data = df, family=cratio(parallel = T, reverse = T))
```