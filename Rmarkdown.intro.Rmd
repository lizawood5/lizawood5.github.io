...
YAMLs are the "set-up" for your Rmd file, indicated by three dashes to begin and end, and it can contain a series of information bits about the Rmarkdown document in between. Initially you'll want to make sure you establish the right `output:`, but afterwards there are advanced features you can add such as `toc:` for Table of Contents, `abstract:`, `keywords:`, etc.  

For more advances additions, what goes in a YAML really depends on the kind of output you have. For basic Word or Notebook outputs, the items above are the crux of it. However, if you start knitting PDFs, this opens up the world of LaTeX into your document.  

A good overview of just how powerful the YAML can be in different contexts can be found here:
  * [YAML Field Guide](https://cran.r-project.org/web/packages/ymlthis/vignettes/yaml-fieldguide.html)  

The remainder of this guide will focus on a `html_output`, but if you want to output as a PDF, note that you will need to set up LaTeX.  
  * [Cookbook on LaTeX](https://bookdown.org/yihui/rmarkdown-cookbook/install-latex.html)  
...
## Writing out texts using the markdown language  

Markdown is how we are able write text into the document as a text editor. The Rmd Cheat Sheet is a good place to start for the basics of the *Markdown* language.
