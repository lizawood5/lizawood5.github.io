...
pp_pred_op = cbind(pp_df, predict(NMP15_op, newdata = pp_df, type = "link", se=TRUE))
pp_pred_snp = cbind(pp_df, predict(NMP15_snp, newdata = pp_df, type = "link", se=TRUE))
...
fit1 = pp_pred_snp$`fitted.values.logitlink(P[Y>1|Y>=1])`
fit2 = pp_pred_snp$`fitted.values.logitlink(P[Y>2|Y>=2])`
se.fit1 = pp_pred_snp$`se.fit.logitlink(P[Y>1|Y>=1])`
se.fit2 = pp_pred_snp$`se.fit.logitlink(P[Y>2|Y>=2])`
...
ppdf_predCIs <- within(pp_pred_snp, {
